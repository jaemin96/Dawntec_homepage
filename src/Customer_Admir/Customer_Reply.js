...
class Customer_Reply extends React.Component {
  constructor(props) {
    super();
  ...
  Updateitem = async () => {
    Axios.post("https://www.dawnth.co.kr:443/api/reply_update", {
      admir_reply_Data: this.state.admir_reply_Data,
      reply: "답변완료",
  ...
  replyChange = (e) => {
    //Change 이벤트를 실행시킨다.
    this.setState({
      admir_reply_Data: e.target.value,
    });
  };
  ...
  componentDidMount() {
    ...
    console.log(this.props.location.state.Question_Data);
          ...
          <div class="Grid4_Main">
            ...
            <div className="detail_btn_container">
              <button onClick={this.Updateitem} className="detaile_btn">
                <Link to="/Customer_Admir/Customer_Admir" className="Link_Ctl">
                  답변하기
                </Link>{" "}
              </button>
